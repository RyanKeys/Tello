...
class Tello:
    ...
    def __init__(self):
        ...
        # Run tello udp receivers in the background
        thread = threading.Thread(target=self.run_udp_receiver, args=())
        state_thread = threading.Thread(target=self.run_state_recv, args=())
        thread.daemon = True
        thread.setName("RECV/RESP Thread")
        state_thread.daemon = True
        state_thread.setName("STATE Thread")
        thread.start()
        state_thread.start()
    ...
    def run_state_recv(self):
        while True:
            try:
                data, _ = self.state_socket.recvfrom(1518)
                state = data.decode(encoding="utf-8")
                state_parse = state.rsplit(";")
                Tello.str_to_int(self, state_parse)
            except Exception as e:
                print(e)

    def str_to_int(self, state_parse):
        pitch = state_parse[5]
        pitch_index = pitch.find(":") + 1
        pitch = pitch[pitch_index:]
        pitch = int(pitch)
        if pitch < -10:
...
class BackgroundFrameRead:
    ...
    def start(self):
        video_thread = threading.Thread(target=self.update_frame, args=())
        video_thread.setName("VIDEO RECV Thread")
        video_thread.start()
        return self
